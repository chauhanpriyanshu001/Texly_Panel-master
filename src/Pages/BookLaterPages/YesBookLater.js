...
const YesBookLater = () => {
  const localValue = sessionStorage.getItem("BookLaterTodayValue");
  ...
  const getTodayBookNow = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/admin/yesterdayBookingsBOOKLATER`, {
        headers: {
          token,
        },
      });
  ...
  useEffect(() => {
    getTodayBookNow();
    sessionStorage.removeItem("BookLaterTodayValue");
  }, []);
  return (
    <div>
      TodayBookNow
