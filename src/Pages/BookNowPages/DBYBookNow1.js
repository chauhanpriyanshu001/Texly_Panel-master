...
const DBYBookNow = () => {
  const localValue = sessionStorage.getItem("BookNowDBYValue");
  ...
  const getTodayBookNow = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/admin/beforeYesterdayBookingsBOOKNOW`, {
        headers: {
          token,
        },
      });
  ...
  useEffect(() => {
    getTodayBookNow();
    sessionStorage.removeItem("BookNowDBYValue");
  }, []);
  return (
    <div>
      TodayBookNow
