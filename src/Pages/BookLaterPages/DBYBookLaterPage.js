...
const DBYBookLaterPage = () => {
  const localValue = sessionStorage.getItem("BookLaterDBYValue");
  ...
  const getTodayBookNow = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/admin/beforeYesterdayBookingsBOOKLATER`, {
        headers: {
          token,
        },
      });
      if (localValue) {
        if (localValue == "Total") {
          setTodayBookNow(res.data?.beforeYesterdayBookings);
        }
  ...
  useEffect(() => {
    getTodayBookNow();
    sessionStorage.removeItem("BookLaterDBYValue");
  }, []);
  return (
    <div>
      TodayBookNow
...
export default DBYBookLaterPage;
